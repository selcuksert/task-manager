...
public class TaskProcessor {
	...
	/**
	 * Processor topology to join task stream with global KTable of user data. When
	 * a message on task topic is consumed it is left joined with global KTable of
	 * users in case of userId of task has matches with the id of user in KTable.
	 * <p>
	 * As the keyspace of user data (key: username) is not so big and has low
	 * cardinality, the un-partitioned version of KTable, GlobalKTable is selected.
	 * <p>
	 * As the detailed task data has high cardinality (key: uuid) and expected to
	 * grow into a large keyspace the output of processing is materialized as KTable
	 *
	...
	public BiFunction<KStream<String, Task>, GlobalKTable<String, User>, KStream<String, DetailedTask>> detail() {
		return (taskStream, userTable) -> taskStream.filter((taskId, task) -> task != null)
	...
	public Function<KStream<String, Task>, KStream<String, Long>> activity() {
		return (taskStream) -> {
			KStream<String, Long> taskCount = taskStream
					...
					.map((taskId, task) -> KeyValue.pair(task.getUserid(), task)).groupByKey()
					.windowedBy(TimeWindows.of(Duration.ofHours(1))).count().toStream()
					.map((userId, task) -> KeyValue.pair(userId.key(), task))
