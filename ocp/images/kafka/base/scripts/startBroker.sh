...
awk -v NODE_HOST=$NODE_HOST -v NODE_PORT=$NODE_PORT '{sub(/#advertised.listeners=PLAINTEXT:\/\/your.host.name:9092/,"advertised.listeners=INTERNAL://"NODE_HOST":"NODE_PORT",EXTERNAL://"NODE_HOST":1"NODE_PORT)}1' | \
awk -v BROKER_ID=$BROKER_ID '{sub(/log.dirs=\/tmp\/kafka-logs/,"log.dirs=/var/lib/kafka/logs/kafka-logs-"BROKER_ID)}1' | \
...
cat ${KAFKA_HOME}/config/server-${BROKER_ID}.properties
